 ...
 * This function normalizes the record map so downstream code works unchanged.
 *
 * v6: recordMap.block[id]           = { role, value: { type, ... } }
 * v7: recordMap.block[id]           = { role, value: { role, value: { type, ... } } }
 *
 * After normalization both look like v6.
 */
function normalizeRecord(record) {
  if (!record) return record;
  const val = record.value;
  // Detect v7 extra envelope: value has both 'role' and 'value' sub-keys
...
export async function getAllPosts({ includePages = false }) {
  ...
  // Normalize v7 double-nesting so all record accesses work as in v6
  ...
  if (rawMetadata?.type !== "collection_view_page" && rawMetadata?.type !== "collection_view") {
    ...
    return []; // return [] instead of null to prevent downstream crashes
  } else {
    // Construct Data
