...
import api, { publicApi } from "@/lib/server/notion-api";
...
async function fetchAllPostsFromNotion() {
  ...
  let rawResponse;
  let lastError;
  for (let attempt = 0; attempt < 3 && !rawResponse; attempt++) {
    try {
      rawResponse = await api.getPage(id);
    } catch (err) {
      lastError = err;
      ...
      if (status === 403) {
        // A stale token_v2 can block an otherwise public page. Retry anonymously.
        try {
          rawResponse = await publicApi.getPage(id);
        } catch (publicErr) {
          console.error(`Failed to fetch public Notion page "${id}" (403).`)
          return null
        }
      }
    }
  ...
  if (!rawResponse) {
    ...
    if (status === 429) {
      console.warn(`Notion rate limited database "${id}"; using empty result for this build.`)
    } else {
      console.error(`Failed to fetch Notion page "${id}":`, lastError)
    }
    return null
  }
