...
async function fetchAllPostsFromNotion() {
  ...
  try {
    ...
    if (status === 403) {
      const hasAccessToken = Boolean(process.env.NOTION_ACCESS_TOKEN?.trim());
      console.error(
        `Failed to fetch Notion page "${id}" (403). ` +
        `${hasAccessToken ? 'The configured NOTION_ACCESS_TOKEN was rejected; refresh the token_v2 cookie and verify the account can access this database.' : 'NOTION_ACCESS_TOKEN is missing; share the database to web or configure a valid token_v2 cookie.'}`
      );
      return null;
    }
