...
export async function getAllPosts({ includePages = false }) {
  ...
  // notion-client v7 wraps each record in an extra {role, value} envelope
  // so the actual collection data is now at .value.value
  const collectionRecord = Object.values(response.collection)[0];
  const collection = collectionRecord?.value?.value ?? collectionRecord?.value;
  if (!collection) {
